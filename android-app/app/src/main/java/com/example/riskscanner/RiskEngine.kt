...
object RiskEngine {
    fun evaluate(s: DeviceSnapshot, kycCountry: String, strict: Boolean = false, today: LocalDate = LocalDate.now()): RiskResult {
        ...
        add("인터넷 연결 검증", if(s.networkValidated==true) "Android 연결 검증 완료" else if(s.networkValidated==false) "검증되지 않음" else "미확인", 5, s.networkValidated==false, "로그인 포털 또는 인터넷 연결을 확인하세요. 연결 검증은 네트워크 안전성 보증이 아닙니다.", s.networkValidated!=null)
        ...
        add("기기 정보", s.deviceModel.ifBlank { "미확인" }, 0, false, "", s.deviceModel.isNotBlank())
