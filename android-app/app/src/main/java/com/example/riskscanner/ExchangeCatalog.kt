...
object ExchangeCatalog {
 fun load(context:Context):List<Exchange> {
  ...
  val byUrl=featured.associateBy{it.infoUrl.trimEnd('/')}
  val byName=featured.associateBy{it.name.lowercase(java.util.Locale.ROOT)}
  val all=directory.entries.map{e->byUrl[e.infoUrl.trimEnd('/')]?:byName[e.name.lowercase(java.util.Locale.ROOT)]?:Exchange(e.name,e.name.take(2),Color(0xFFD9B56D),infoUrl=e.infoUrl)}
  return (featured+all).distinctBy{it.infoUrl.trimEnd('/')}.plus(Exchange("기타 거래소","+",Color(0xFFD9B56D)))
 }
}
