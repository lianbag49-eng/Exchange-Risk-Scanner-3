 ...
 Dialog(onDismissRequest=close,properties=DialogProperties(usePlatformDefaultWidth=false)){
  Surface(modifier=Modifier.fillMaxSize(),color=Color(0xFF101115),contentColor=Color.White){
   Column(Modifier.fillMaxSize().navigationBarsPadding().imePadding().padding(20.dp).verticalScroll(rememberScrollState()),verticalArrangement=Arrangement.spacedBy(12.dp)){
    ...
    report?.let{r->HorizontalDivider();AppDiagnosticSummary(r);Button(onClick={try{onSave(r);if(saveLabel!=null)message="원인 조사 이력에 암호화해 저장했습니다"}catch(e:Exception){message=e.message?:"결과 저장 실패"}},modifier=Modifier.fillMaxWidth()){Text(saveLabel?:if(persisted)"암호화된 계정 기록에 결과 저장" else "현재 세션에 결과 보관")}}
