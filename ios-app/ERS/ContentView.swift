...
private let bg=Color(red:0.02,green:0.04,blue:0.065)
private let panel=Color(red:0.045,green:0.082,blue:0.12)
private let panel2=Color(red:0.07,green:0.12,blue:0.17)
private let line=Color(red:0.14,green:0.22,blue:0.29)
private let gold=Color(red:0.85,green:0.71,blue:0.43)
private let gold2=Color(red:0.95,green:0.85,blue:0.62)
private let cyan=Color(red:0.33,green:0.85,blue:1)
private let muted=Color(red:0.57,green:0.65,blue:0.71)
private let green=Color(red:0.34,green:0.84,blue:0.60)
private let amber=Color(red:0.96,green:0.71,blue:0.30)
private let red=Color(red:1,green:0.43,blue:0.46)
...
struct ContentView: View {
    ...
    var body: some View {
        ZStack {
            LinearGradient(colors:[Color(red:0.035,green:0.08,blue:0.12),bg,Color.black],startPoint:.top,endPoint:.bottom).ignoresSafeArea()
            VStack(spacing:0) {
                Header()
                TabView(selection:$tab) {
                    HomeView(start:{$tab.wrappedValue=1}).tag(0)
                    ScanView().tag(1)
                    HistoryView().tag(2)
                    SettingsView().tag(3)
    ...
    @ViewBuilder func Nav(_ title:String,_ index:Int)->some View {
        Button { tab=index } label: {
            ...
            }.frame(maxWidth:.infinity).padding(.vertical,7).background(tab==index ? gold.opacity(0.1):.clear).clipShape(RoundedRectangle(cornerRadius:13))
        }
    }
}
...
struct Header: View {
    var body: some View {
        HStack {
            ZStack { RoundedRectangle(cornerRadius:15).fill(LinearGradient(colors:[gold.opacity(.25),panel2],startPoint:.topLeading,endPoint:.bottomTrailing));RoundedRectangle(cornerRadius:15).stroke(gold.opacity(.7));Text("E").font(.system(size:24,weight:.black)).foregroundStyle(gold2) }.frame(width:48,height:48)
            ...
            Spacer()
            Text("SYSTEM SECURE").font(.system(size:8,weight:.bold)).foregroundStyle(green).padding(.horizontal,9).padding(.vertical,6).background(green.opacity(.09)).clipShape(Capsule()).overlay(Capsule().stroke(green.opacity(.3)))
    ...
    var body:some View {
        ScrollView {
            VStack(alignment:.leading,spacing:14) {
                ...
                CardBox {
                    HStack {
                        VStack(alignment:.leading){LabelText("CURRENT STATUS");Text(latest?.level ?? "READY").font(.system(size:34,weight:.black)).foregroundStyle(levelColor(latest?.level));Text(latest.map{$0.exchange.name+" · UID "+mask($0.uid)} ?? "첫 번째 보안 스캔을 시작하세요").font(.caption).foregroundStyle(muted)}
    ...
    var body:some View {
        ScrollView {
            VStack(alignment:.leading,spacing:14) {
                ...
                CardBox {InfoRow("Application","Exchange Risk Scanner");InfoRow("Version","1.4 iOS");InfoRow("Engine","ERS Device Guard");InfoRow("Data Mode","On-device only")}
...
struct RecordRow:View{let r:ScanRecord;let tap:()->Void;init(_ r:ScanRecord,_ tap:@escaping()->Void){self.r=r;self.tap=tap};var body:some View{Button(action:tap){HStack{ZStack{RoundedRectangle(cornerRadius:12).fill(gold.opacity(.1));Text(r.exchange.mark).font(.caption).fontWeight(.black).foregroundStyle(gold2)}.frame(width:42,height:42);VStack(alignment:.leading){Text(r.exchange.name).fontWeight(.bold);Text("UID "+mask(r.uid)).font(.caption2).foregroundStyle(muted)};Spacer();VStack{Text("\(r.score)").font(.system(size:20,weight:.black));Text(r.level).font(.system(size:9,weight:.bold))}.foregroundStyle(levelColor(r.level))}}.buttonStyle(.plain).padding(14).background(panel).clipShape(RoundedRectangle(cornerRadius:18)).overlay(RoundedRectangle(cornerRadius:18).stroke(line))}}
func Notice(_ s:String,_ c:Color)->some View{Text(s).font(.caption).foregroundStyle(c==red ? red:muted).padding(14).frame(maxWidth:.infinity,alignment:.leading).background(c.opacity(.08)).clipShape(RoundedRectangle(cornerRadius:16)).overlay(RoundedRectangle(cornerRadius:16).stroke(c.opacity(.25)))}
func levelColor(_ s:String?)->Color{s=="HIGH" ? red:s=="MEDIUM" ? amber:s=="LOW" ? green:gold}
func mask(_ s:String)->String{s.count<=4 ? s:String(s.prefix(2))+String(repeating:"•",count:min(6,s.count-4))+String(s.suffix(2))}
