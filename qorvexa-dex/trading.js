...
import {boundedPrice,leverageRequest,finite,fundingCashflow} from './terminal-core.js';
...
const errorText=e=>String(e?.shortMessage||e?.message||'Request failed').slice(0,400);
...
function validateReduce(order,pos){if(!order.r)return;if(!pos||!finite(pos.szi)||Number(pos.szi)===0||order.b!==(Number(pos.szi)<0)||Number(order.s)>Math.abs(Number(pos.szi)))throw Error('Reduce-only size and side must match your existing position');}
...
function positionAction(pos,type){if(market?.network!=='testnet'||market.market?.value!==pos.coin)throw Error('Select '+pos.coin+' on testnet first');$('tradeSide').value=Number(pos.szi)>0?'sell':'buy';$('tradeType').value=type;$('tradeSize').value=String(Math.abs(Number(pos.szi)));$('tradeReduce').checked=true;clearPending();updateTicket();$('tradeSize').scrollIntoView({block:'center',behavior:'smooth'});status(type==='Market'?'Review the reduce-only close order before signing.':'Enter the trigger price, then review.');}
