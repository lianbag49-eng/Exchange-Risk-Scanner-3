...
function availability(){$('tradeReview').disabled=busy||!client||!freshMarket(market);$('tradeConnect').disabled=busy;$('walletProvider').disabled=busy;$('tradeSubmit').disabled=busy;}
function disconnect(){window.dispatchEvent(new CustomEvent('beltrix:wallet',{detail:{account:null}}));$('tradeConnect').textContent='Connect wallet';epoch++;account=null;client=null;pending=null;$('tradeAccount').textContent='Connect your wallet';$('tradeBalances').replaceChildren();$('tradeOrders').replaceChildren();$('tradePositions').replaceChildren();$('tradeDialog').close();availability()}
...
$('tradeConnect').onclick=async()=>{if(busy)return;window.openPage?.('markets');if(client){disconnect();status('Wallet disconnected');return}busy=true;availability();try{
 const chosen=$('walletProvider').value==='okx'?window.okxwallet:window.ethereum;
 ...
 $('tradeConnect').textContent=account.slice(0,6)+'…'+account.slice(-4)+' ×';$('tradeAccount').textContent=account+' · Hyperliquid testnet';window.dispatchEvent(new CustomEvent('beltrix:wallet',{detail:{account}}));status('Connected · Testnet funds required');await refresh();
