...
const box=$('marketTrades');box.replaceChildren();for(const t of trades.slice(-10).reverse()){const row=document.createElement('div');row.className='row space pair';row.textContent=`${new Date(t.time).toLocaleTimeString("en-US")}  ${t.side==='B'?'Buy':'Sell'}  ${t.px} · ${t.sz}`;box.append(row)}}
...
async function selectMarket(){
 const token=++generation;cleanup();controller=new AbortController();candles=[];book=null;trades=[];lastTradeTime=0;streamReceived=0;emit();paintBook();lastUpdate=0;$('marketPrice').textContent='—';$('marketOHLC').textContent='';draw();status('Connecting');
