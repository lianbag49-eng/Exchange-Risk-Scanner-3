...
export function createService({apiKey,model,tokens,review=reviewImage,diagnose=diagnoseImage,checkProvider=providerStatus,preflight=reviewPreflight,causes=reviewCauses,identity=readIdentity,identitySettings=null,now=Date.now}){
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  try{
   ...
   const body=req.url==='/v1/causes'?validateCauses(input):req.url==='/v1/identity/status'?validateIdentity(input):req.url==='/v1/preflight'?validatePreflight(input):req.url==='/v1/diagnostics'?validateDiagnostic(input):validateInput(input);for(const [id,expiry] of seen)if(expiry<time)seen.delete(id);
   const key=user.id+':'+body.requestId;if(seen.has(key))throw new ReviewError('duplicate_review',409);seen.set(key,time+300000);
   if(active>=4)throw new ReviewError('review_rate_limit',429);active++;claimed=true;const result=req.url==='/v1/identity/status'?await identity(body,{reviewer:user.id,config:identitySettings}):await (req.url==='/v1/causes'?causes:req.url==='/v1/preflight'?preflight:req.url==='/v1/diagnostics'?diagnose:review)(body,{apiKey,model});send(200,result);
