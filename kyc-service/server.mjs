...
export function createService({apiKey,model,tokens,review=reviewImage,diagnose=diagnoseImage,checkProvider=providerStatus,preflight=reviewPreflight,now=Date.now}){
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  if(req.method!=='POST'||!['/v1/kyc/reviews','/v1/diagnostics','/v1/connection-check','/v1/preflight'].includes(req.url))return send(404,{error:'not_found'});
  ...
  try{
   ...
   const body=req.url==='/v1/preflight'?validatePreflight(input):req.url==='/v1/diagnostics'?validateDiagnostic(input):validateInput(input);for(const [id,expiry] of seen)if(expiry<time)seen.delete(id);
   const key=user.id+':'+body.requestId;if(seen.has(key))throw new ReviewError('duplicate_review',409);seen.set(key,time+300000);
   if(active>=4)throw new ReviewError('review_rate_limit',429);active++;claimed=true;const result=await (req.url==='/v1/preflight'?preflight:req.url==='/v1/diagnostics'?diagnose:review)(body,{apiKey,model});send(200,result);
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){
 const server=createService({apiKey:process.env.OPENAI_API_KEY,model:process.env.OPENAI_MODEL,tokens:process.env.ERS_REVIEW_TOKENS?JSON.parse(process.env.ERS_REVIEW_TOKENS):process.env.ERS_REVIEW_TOKEN?{reviewer:process.env.ERS_REVIEW_TOKEN}:{}});
