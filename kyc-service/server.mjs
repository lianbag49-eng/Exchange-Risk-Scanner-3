...
export function createService({apiKey,model,tokens,review=reviewImage,diagnose=diagnoseImage,checkProvider=providerStatus,now=Date.now}){
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  if(req.method!=='POST'||!['/v1/kyc/reviews','/v1/diagnostics','/v1/connection-check'].includes(req.url))return send(404,{error:'not_found'});
  ...
  try{
   ...
   const body=req.url==='/v1/diagnostics'?validateDiagnostic(input):validateInput(input);for(const [id,expiry] of seen)if(expiry<time)seen.delete(id);
   const key=user.id+':'+body.requestId;if(seen.has(key))throw new ReviewError('duplicate_review',409);seen.set(key,time+300000);
   if(active>=4)throw new ReviewError('review_rate_limit',429);active++;claimed=true;const result=await (req.url==='/v1/diagnostics'?diagnose:review)(body,{apiKey,model});send(200,result);
